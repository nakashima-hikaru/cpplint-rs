...
## 2026-07-28 - Avoid PathBuf::set_extension() bugs for stems with dots
**Learning:** In Rust, `PathBuf::set_extension()` *replaces* the existing extension if the file stem contains a dot (e.g., `foo.pb` becomes `foo.h` rather than `foo.pb.h`). When optimizing `format!("{}.{}", stem, ext)` allocations where `stem` may contain dots (common in generated C++ files like `foo.pb.cc`), using `PathBuf::set_extension` causes severe logic bugs.
**Action:** When replacing `format!("{}.{}", stem, ext)` in hot loops for path generation, avoid `PathBuf::set_extension`. Instead, pre-allocate a reusable `String` buffer (e.g., `String::with_capacity(stem.len() + 8)`), use `.clear()`, and manually `.push_str()` the components before using it with `PathBuf::push()`.

...
## 2026-07-28 - Avoid format!() when checking static Path::extension
**Learning:** Found a micro-performance bottleneck in `get_include_kind` where `include.extension()` was being converted to a string and prepended with a dot via `format!(".{}", ext)` dynamically just to check against standard extensions using `matches!`. This caused a heap allocation inside a highly-executed path analyzing C++ include statements.
**Action:** When matching `Path::extension()` against known values on hot paths in Rust, extract the raw `&str` slice and match it directly against string literals (e.g., `matches!(ext, "hh" | "hpp")`). Do not allocate a new string merely to append a dot.
